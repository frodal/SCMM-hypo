...
!-----------------------------------------------------------------------
      if((steptime.eq.zero).and.(totaltime.eq.zero))then
          do km = 1, nblock
! Assume small elastic deformation in the calculation of the initial elastic wave speeds
...
!-----------------------------------------------------------------------
     ...
     +      2.d0*(stressOld(km,4)+stressNew(km,4))*strainInc(km,4) +
     +      2.d0*(stressOld(km,5)+stressNew(km,5))*strainInc(km,5) +
     +      2.d0*(stressOld(km,6)+stressNew(km,6))*strainInc(km,6) )
!
